        ...
        if( isset($product) && !empty($product) ) :
            ...
            if( $item->get_type() === 'line_item' ){
                ...
                if ( in_array( 'sitepress-multilingual-cms/sitepress.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && function_exists('icl_object_id') && $is_translated ) {
                    ...
                    $inventory_refs = get_post_meta( $order_id, 'order_item_'.$item_id.'_'.$order_lang.'_inventory_ref', true );
                }else{
                ...
                if( !empty($inventory_refs) ){
                    echo '<div class="rnb-inventory-ref"> <span class="title"> '.esc_html__( 'Inventory Reference', 'redq-rental' ).' : </span>';
                        foreach ($inventory_refs as $key => $inventory_id) {
                            ...
                            echo '<a target="_blank" href="'.get_edit_post_link($inventory_id).'">'.get_the_title( $inventory_id ).'</a>'.$last;
                        }
                    echo '</div>';
                }
            }
        ...
        if( isset($items) && !empty($items) ) :
            foreach ($items as $item_key => $item_value) {
                ...
                if( isset( $product_type) && $product_type === 'redq_rental' ){
                    if( metadata_exists('post', $order_id, 'order_item_'.$item_id.'_blocked_date' ) ) {
                        $booked_dates_ara = get_post_meta( $order_id, 'order_item_'.$item_id.'_blocked_date', true );
                    }

                    if( $new_status === 'cancelled' && $old_status !== 'cancelled' ) :
                        $this->rnb_sync_order_dates( $order_id, $item_id, $product_id, $booked_dates_ara );
                    endif;
                }
            }
        endif;
