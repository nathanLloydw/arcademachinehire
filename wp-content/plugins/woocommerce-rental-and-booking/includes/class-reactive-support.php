...
class Redq_Rental_Reactive {
     ...
     public function __construct(){
        add_filter( 're_update_meta_fields', array( $this, 'redq_rental_re_update_meta_fields' ),1 );
        add_filter( 're_post_type_for_posts', array( $this, 'redq_rental_re_post_type_for_posts' ),1 );
        add_filter( 're_get_post_types_for_taxonomy', array( $this, 'redq_rental_re_get_post_types_for_taxonomy' ),1 );
        add_filter( 're_preview_post_terms', array( $this, 'redq_rental_re_preview_post_terms' ),1, 3 );
        add_filter( 're_preview_post_meta', array( $this, 'redq_rental_re_preview_post_meta' ),1, 4 );
        add_filter( 're_preview_posts', array( $this, 'redq_rental_re_preview_posts' ),1 );
        add_filter( 're_all_builder_data', array( $this, 'redq_rental_re_all_builder_data' ),1 );
    }
    ...
    public function redq_rental_re_update_meta_fields($builder_data){
        ...
        $builder_data[0]['fields'][] = array(
            'linear' => array(
                array( 'type'=>'select','id' => 'rebuilder_rnb_search' , 'title' => 'Choose To Enable RnB Search' ,
                'options' => array(
     ...
     * If rnb search ten retreive prdouct and inventory post-types meta-values
     *
     * @since 3.0.5
     * @version 3.0.5
     * @var object
                    ...
                    foreach ($pre_value as $sub_key => $sub_value) {
                        if(array_key_exists('redq_block_dates_and_times', $sub_value->meta)){
                            ...
                            if(isset($product_type) && $product_type === 'redq_rental'){
                                ...
                                $starting_block_days = reddq_rental_staring_block_days($sub_value->ID);
