			...
			case 'attributes':
				...
				if(isset($unique_selected_terms) && is_array($unique_selected_terms)){
					foreach ($unique_selected_terms as $key => $value) {
						...
						$attributes[$key]['name'] = isset($name) && !empty($name) ? $name : $value->name;
						$attributes[$key]['value'] = $avalue;
						$attributes[$key]['selected_icon'] = $selected_icon === 'image' ? $selected_icon : 'icon';
						$attributes[$key]['icon'] = $icon;
						$attributes[$key]['image'] = $image;
						$attributes[$key]['highlighted'] = $highlighted === 'yes' ? $highlighted : 'no';
					}
				}
