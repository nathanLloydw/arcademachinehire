	...
	*/
	...
	if (!function_exists('sf_blog_aux')) {
		function sf_blog_aux($width) {
			...
			// CATEGORIES
			$blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="categories"><i class="fas fa-list"></i>'.__("Categories", 'neighborhood').'</a>';
			
			// TAGS
			$blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="tags"><i class="fas fa-tags"></i>'.__("Tags", 'neighborhood').'</a>';
			
			// SEARCH FORM
			$blog_aux_output .= '<li><form method="get" class="search-form" action="'. home_url().'/">';
			$blog_aux_output .= '<input type="text" placeholder="'. __("Search", 'neighborhood') .'" name="s" />';
			$blog_aux_output .= '</form></li>';
			
			// ARCHIVES
			$blog_aux_output .= '<li><a href="#" class="blog-slideout-trigger" data-aux="archives"><i class="fas fa-list"></i>'.__("Archives", 'neighborhood').'</a>';
			
			// RSS LINK
			if ($rss_feed_url != "") {
			$blog_aux_output .= '<li><a href="'.$rss_feed_url.'" class="rss-link" target="_blank"><i class="fas fa-rss"></i>'.__("RSS", 'neighborhood').'</a>';
			}
