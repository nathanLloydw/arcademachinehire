	...
	if ( !function_exists('sf_link_post') ) {
		function sf_link_post($postID) {
			...
			if (function_exists('get_the_post_format_url')) {
				...
				$link = '<a href="'.esc_url($link).'" target="_blank" class="link-post-link"><i class="fas fa-link"></i>'.$link.'</a>';
		...
		function sf_get_post_item($postID, $blog_type, $show_title = "yes", $show_excerpt = "yes", $show_details = "yes", $excerpt_length = "20", $content_output = "excerpt", $show_read_more = "no") {
			...
			if ($thumb_type == "video") {
				...
				if ($image) {
					...
					$item_figure .= '<i class="fas fa-'.$item_icon.'"></i>';
					$item_figure .= '</div></div>';
					}
			...
			if ($blog_type == "masonry") {
				...
				if ($post_format == "quote") {
					$post_item .= '<div class="quote-display"><i class="fas fa-quote-left"></i></div>';
				} else {
				...
				$post_item .= '<a class="read-more" href="'.$post_permalink.'">'.__("Read more", 'neighborhood').'<i class="fas fa-angle-right"></i></a>';
				}
				...
				if ( comments_open() ) {
				$post_item .= '<a href="'.$post_permalink.'#comment-area"><i class="fas fa-comments"></i><span>'. $post_comments .'</span></a> ';
				}
				if (function_exists( 'lip_love_it_link' )) {
				$post_item .= lip_love_it_link(get_the_ID(), '<i class="fas fa-heart"></i>', '<i class="fas fa-heart"></i>', false);
				}
				$post_item .= '</div>';				
				$post_item .= '</div>';
				}
			...
			// MINI STYLING
				...
				if ($show_details == "yes") {
				$post_item .= '<div class="blog-item-details">'. sprintf(__('By <a href="%2$s">%1$s</a> on %3$s', 'neighborhood'), $post_author, get_author_posts_url(get_the_author_meta( 'ID' )), $post_date) .'</div>';
			...
			// STANDARD STYLING
				...
				$post_item .= '<span class="standard-post-author-name" itemprop="author">'.__("Posted by", 'neighborhood').' '.$post_author.'</span>';
				$post_item .= '</div>';
				} else if ($sidebars == "right-sidebar" || $sidebars == "left-sidebar" || $sidebars == "one-sidebar") {
				$post_item .= '<div class="standard-post-author span1">';
				if(function_exists('get_avatar')) {
				...
				if ($show_details == "yes") {
					...
					if ( comments_open() ) {
						$post_item .= '<div class="comments-wrapper"><a href="'.$post_permalink.'#comment-area"><i class="fas fa-comments"></i><span>'. $post_comments .'</span></a></div>';
					}
