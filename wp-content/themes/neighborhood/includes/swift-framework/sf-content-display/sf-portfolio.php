	...
	*/
				...
				if ($thumb_type != "none") {
					...
					if ($thumb_type == "video") {
						...
						if($image) {
							...
							if ($display_type == "standard") {
							$portfolio_items_output .= '<i class="fas fa-'.$item_icon.'"></i>'. "\n";
							} else {
							$portfolio_items_output .= '<h4 itemprop="name headline">'.$item_title.'</h4>'. "\n";
							$portfolio_items_output .= '<i class="fas fa-'.$item_icon.' small-icon"></i>'. "\n";
							}
	...
	if ( !function_exists('sf_portfolio_filter') ) {
		function sf_portfolio_filter() {
		    ...
		    $filter_output .= '<a href="#" class="select"><i class="fas fa-align-justify"></i>'. __("Filter our work", 'neighborhood') .'</a>'. "\n";
		    $filter_output .= '<div class="filter-slide-wrap">'. "\n";
		    $filter_output .= '<ul class="portfolio-filter filtering row clearfix">'. "\n";
		    $filter_output .= '<li class="all selected span2"><a data-filter="*" href="#"><span class="item-name">'. __("All", 'neighborhood').'</span><span class="item-count">0</span></a></li>'. "\n";
