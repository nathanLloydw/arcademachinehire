		...
		if( $slider_items->have_posts() ) {
			...
			while ( $slider_items->have_posts() ) : $slider_items->the_post();
				...
				if ($image) {
					...
					if ($post_client != "") {
					$output .= '<span class="item-client">'.__("Client: ", 'neighborhood').$post_client.'</span>'. "\n";
					$output .= '<span class="item-categories">'.$post_categories.'</span>'. "\n";
					} else {
					$output .= '<span class="item-author">'.__("Posted by ", 'neighborhood').$post_author.'</span>'. "\n";
					$output .= '<span class="item-categories">'.$post_categories.'</span>'. "\n";
					}
					$output .= '</div>';
					if ( comments_open() ) {
						$output .= '<div class="comment-chart chart" data-percent="1" data-count="'.$post_comments.'" data-barcolor="'.$secondary_accent_color.'"><span>0</span><i class="fas fa-comments"></i></div>'. "\n";
					}
					if (function_exists( 'lip_get_love_count' )) {
					$output .= '<div class="loveit-chart chart" data-percent="1" data-count="'.lip_get_love_count($post->ID).'" data-barcolor="'.$accent_color.'"><span>0</span><i class="fas fa-heart"></i></div>'. "\n";
					}
					$output .= '</div>'. "\n";
					$output .= '</div>'. "\n";
					$output .= '<div class="flex-caption-headline clearfix">'. "\n";
					$output .= '<h4><a href="'.$post_permalink.'"><span>'. $post_title .'</span><i class="fas fa-angle-right"></i></a></h4>'. "\n";
					$output .= '</div></div></div>'. "\n";
					...
					if ( comments_open() ) {
						$output .= '<div class="comment-chart fw-chart chart" data-percent="1" data-count="'.$post_comments.'" data-barcolor="'.$secondary_accent_alt_color.'"><span>0</span><i class="fas fa-comments"></i></div>'. "\n";
					}
					if (function_exists( 'lip_get_love_count' )) {
					$output .= '<div class="loveit-chart fw-chart chart" data-percent="1" data-count="'.lip_get_love_count($post->ID).'" data-barcolor="'.$accent_color.'"><span>0</span><i class="fas fa-heart"></i></div>'. "\n";
					}	
					$output .= '</div>'. "\n";
					$output .= '<div class="details clearfix">'. "\n";
					$output .= '<span class="date">'.$post_date.'</span>'. "\n";
