...
class RWMB_File_Field extends RWMB_Field
{
	...
	public static function admin_enqueue_scripts()
	{
		...
		if ( ! $wp_scripts->get_data( 'rwmb-file', 'data' ) )
		{
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'neighborhood' ),
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'neighborhood' ),
			) );
		}
	}
	...
	public static function wp_ajax_delete_file()
	{
		...
		if ( $success )
			wp_send_json_success();
		wp_send_json_error( __( 'Error: Cannot delete file', 'neighborhood' ) );
	}
	...
	{
		$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'neighborhood' ), $field );
		$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'neighborhood' ), $field );
	...
	{
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'neighborhood' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'neighborhood' ) );
