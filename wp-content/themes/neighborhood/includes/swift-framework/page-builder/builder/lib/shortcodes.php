	...
	*/
	            ...
	            foreach ( $this->settings['params'] as $param ) {
	                if ( $param['param_name'] != 'content' ) {
	                    ...
	                    if ( isset($param['value']) ) {
	                        $shortcode_attributes[$param['param_name']] = is_string($param['value']) ? __($param['value'], 'neighborhood') : $param['value'];
	                    } else {
	                        $shortcode_attributes[$param['param_name']] = '';
	                    }
	                } else if ( $param['param_name'] == 'content' && $content == NULL ) {
	                    $content = __($param['value'], 'neighborhood');
	                }
	            }
	...
	---------------------------------------------------------- */
	    public function getColumnControls($controls) {
	        ...
	        $controls_column_size = ' <div class="column_size_wrapper"> <a class="column_decrease" href="#" title="'.__('Decrease width', 'neighborhood').'"></a> <span class="column_size">%column_size%</span> <a class="column_increase" href="#" title="'.__('Increase width', 'neighborhood').'"></a> </div>';
	
	        $controls_edit = ' <a class="column_edit" href="#" title="'.__('Edit', 'neighborhood').'"></a>';
	        $controls_popup = ' <a class="column_popup" href="#" title="'.__('Pop up', 'neighborhood').'"></a>';
	        $controls_delete = ' <a class="column_clone" href="#" title="'.__('Clone', 'neighborhood').'"></a> <a class="column_delete" href="#" title="'.__('Delete', 'neighborhood').'"></a>';
	        // $delete_edit_row = '<a class="row_delete" title="'.__('Delete %element%', 'neighborhood').'">'.__('Delete %element%', 'neighborhood').'</a>';
	...
	abstract class SwiftPageBuilderShortcode_UniversalAdmin extends SwiftPageBuilderShortcode {
	    ...
	    public function contentAdmin($atts,  $content) {
	        ...
	        if ( isset($this->settings['params']) ) {
	            ...
	            //$output .= '<div class="span12 spb_edit_form_elements"><h2>'.__('Edit', 'neighborhood').' ' .__($this->settings['name'], 'neighborhood').'</h2>';
				$output .= '<div class="spb_edit_form_elements">';
				
				$output .= '<div id="edit-modal-header">';
				$output .= '<h2>'.__('Edit', 'neighborhood').' ' .__($this->settings['name'], 'neighborhood').'</h2>';
				$output .= '<div class="edit_form_actions"><a href="#" class="spb_save_edit_form button-primary">'. __('Save', 'neighborhood') .'</a></div>';
				$output .= '</div>';
	    ...
	    protected function singleParamEditHolder( $param, $param_value ) {
            ...
            if ( $param['type'] == "section" ) {
                ...
                $output .= '<div class="span12 spb_element_section">' . __( $param['heading'], 'neighborhood' ) . '</div>';

            } else {

                $output .= '<label class="spb_element_label">' . __( $param['heading'], 'neighborhood' ) . '</label>';

                $output .= '<div class="edit_form_line">';
                $output .= $this->singleParamEditForm( $param, $param_value );
                $output .= ( isset( $param['description'] ) ) ? '<span class="description">' . __( $param['description'], 'neighborhood' ) . '</span>' : '';
                $output .= '</div>';
	    ...
	    protected function singleParamEditForm( $param, $param_value ) {
            ...
            if ( $param['type'] == 'textfield' ) {
                $value = __( $param_value, 'neighborhood' );
                $value = $param_value;

                if ( $param['param_name'] == 'address' && $param_value == __( 'Click the edit button to change the map pin details.', 'neighborhood' ) ) {
                    $param_line .= '<input name="' . $param['param_name'] . '" class="spb_param_value spb-textinput ' . $param['param_name'] . ' ' . $param['type'] . '" type="text" value="" data-previous-text="' . __( 'Click the edit button to change the map pin details.', 'neighborhood' ) . '"/>';
                } else {
	            ...
	            foreach ( $param['value'] as $text_val => $val ) {
	                ...
	                $text_val = __( $text_val, 'neighborhood' );
	                $checked = '';
	                if ( $val == $param_value ) {
                ...
                // $param_value = __($param_value, 'neighborhood');
                ...
                $param_line .= '<a class="button gallery_widget_add_images" href="#" title="' . __( 'Add images', 'neighborhood' ) . '">' . __( 'Add images', 'neighborhood' ) . '</a>';
                ...
                $param_line .= '<input type="hidden" class="spb_param_value gallery_widget_attached_images_ids ' . $param['param_name'] . ' ' . $param['type'] . '" name="' . $param['param_name'] . '" value="' . $param_value . '" />';
                $param_line .= '<a class="button gallery_widget_add_images" href="#" use-single="true" title="' . __( 'Add image', 'neighborhood' ) . '" data-uploader_title="' . __( 'Add image', 'neighborhood' ) . '">' . __( 'Add image', 'neighborhood' ) . '</a>';
                $param_line .= '<div class="gallery_widget_attached_images">';
                $param_line .= '<ul class="gallery_widget_attached_images_list">';
                $param_line .= ( $param_value != '' ) ? spb_fieldAttachedImages( explode( ",", $param_value ) ) : '';
                $param_line .= '</ul>';
                ...
                foreach ( $sidebars as $sidebar ) {
                    ...
                    $sidebar_name = __( $sidebar["name"], 'neighborhood' );
	    ...
	    protected function getTinyHtmlTextArea($param = array(), $param_value) {
	        ...
	        if ( function_exists('wp_editor') ) {
	            $default_content = __($param_value, 'neighborhood');
	            $output_value = '';
	            // WP 3.3+
	            ob_start();
	...
	class SwiftPageBuilderShortcode_Settings extends SwiftPageBuilderShortcode_UniversalAdmin {
	    ...
	    public function contentAdmin($atts, $content) {
	        ...
	        if ( isset($this->settings['params']) ) {
				...
				$output .= '<div id="edit-modal-header">';
				$output .= '<h2>'.__('Edit', 'neighborhood').' ' .__($this->settings['name'], 'neighborhood').'</h2>';
				$output .= '<div class="edit_form_actions"><a href="#" id="cancel-background-options">' . __('Cancel', 'neighborhood') . '</a><a href="#" class="spb_save_edit_form button-primary">'. __('Save', 'neighborhood') .'</a></div>';
				$output .= '</div>';
