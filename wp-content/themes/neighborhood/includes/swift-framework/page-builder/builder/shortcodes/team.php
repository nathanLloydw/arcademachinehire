...
class SwiftPageBuilderShortcode_team extends SwiftPageBuilderShortcode {
    ...
    protected function content($atts, $content = null) {
			...
			while ( $team_members->have_posts() ) : $team_members->the_post();
				...
				if ($member_email) {
					$items .= '<li><span>'.__("E:", 'neighborhood').'</span><a href="mailto:'.$member_email.'">'.$member_email.'</a></li>';
				}
				if ($member_phone) {
					$items .= '<li><span>'.__("T:", 'neighborhood').'</span>'.$member_phone.'</li>';
				}
				$items .= '</ul>'; 
				}
...
SPBMap::map( 'team', array(
    ...
    "params"	=> array(
        ...
        array(
            ...
            "value" => "12",
            "description" => __("The number of team members to show per page.", 'neighborhood')
        ),
        array(
            "type" => "select-multiple",
            "heading" => __("Team category", 'neighborhood'),
            "param_name" => "category",
            "value" => get_category_list('team-category'),
            "description" => __("Choose the category for the portfolio items.", 'neighborhood')
		),
		array(
		    "type" => "dropdown",
		    "heading" => __("Social icon type", 'neighborhood'),
		    "param_name" => "social_icon_type",
		    "value" => array(__('Dark', 'neighborhood') => "dark", __('Light', 'neighborhood') => "light", __('Coloured', 'neighborhood') => "coloured"),
		    "description" => __("Choose the social icon type to show.", 'neighborhood')
		),
		array(
            "type"        => "buttonset",
            "heading"     => __( "Profile Link", 'neighborhood' ),
            "param_name"  => "profile_link",
            "value"       => array(
                __( 'Yes', 'neighborhood' ) => "yes",
                __( 'No', 'neighborhood' )  => "no"
            ),
            "description" => __( "Select if you'd like the team members to link through to the profile page.", 'neighborhood' )
        ),
		array(
		    "type" => "dropdown",
		    ...
		    "description" => __("Show testimonial pagination.", 'neighborhood')
		),
        array(
            "type" => "textfield",
