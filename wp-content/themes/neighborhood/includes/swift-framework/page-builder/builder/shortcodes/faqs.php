...
class SwiftPageBuilderShortcode_faqs extends SwiftPageBuilderShortcode {
    ...
    public function content( $atts, $content = null ) {
        ...
        $output = $items_nav = $items = '';
       	...
       	$items_nav .= '<h3>'.__("Browse F.A.Q. Topics", 'neighborhood').'</h3>';
       	
       	$items_nav .= '<ul class="faqs-nav clearfix">';
       	foreach ($cats as $cat) {
       		$items_nav .= '<li><a href="#'.$cat->slug.'"><i class="fas fa-list"></i>'.$cat->name.'<span class="count">'.$cat->count.'</span></a></li>';
       	}
       	$items_nav .= '</ul>';
		...
		foreach ($cats as $cat) {
			...
			$items .= '<div class="spb_divider go_to_top_icon1 spb_content_element "><a class="animate-top" href="#"><i class="fas fa-arrow-up"></i></a></div>';
			$items .= '</ul>';
