...
class SwiftPageBuilderShortcode_jobs_overview extends SwiftPageBuilderShortcode {
    ...
    public function content( $atts, $content = null ) {
        ...
        if ($view_all_link == "yes") {
        	...
        	$jobs_page = __($options['jobs_page'], 'neighborhood');
        	if ($jobs_page) {
        	...
        	if ($jobs_page && isset($jobs_page_title)) {
	        	$items .= '<a href="'.get_permalink($jobs_page_id).'" class="read-more">'.__("View all vacancies", 'neighborhood').' ['.$current_jobs.']<i class="fas fa-angle-right"></i></a>';
        	}
        }
...
SPBMap::map( 'jobs_overview', array(
    "name"		=> __("Jobs Overview", 'neighborhood'),
    "base"		=> "jobs_overview",
    "class"		=> "",
    ...
    "params"	=> array(
    	...
    	array(
    	    "type" => "textfield",
    	    "heading" => __("Intro Text", 'neighborhood'),
    	    "param_name" => "job_text",
    	    "value" => "",
    	    "description" => __("Enter the intro text for the jobs overview.", 'neighborhood')
    	),
        array(
            "type" => "textfield",
            "class" => "",
            ...
            "value" => "3",
            "description" => __("The number of jobs to show in the overview list. Leave blank to show ALL jobs.", 'neighborhood')
        ),
        array(
            "type" => "dropdown",
        ...
        array(
            "type" => "dropdown",
            "heading" => __("View all link", 'neighborhood'),
            "param_name" => "view_all_link",
            "value" => array(__('Yes', 'neighborhood') => "yes", __('No', 'neighborhood') => "no"),
            "description" => __("Show the view all jobs link. Make sure you have selected the page within theme options for this to work.", 'neighborhood')
        ),
        array(
            "type" => "textfield",
