    ...
    protected function content($atts, $content = null) {
    		...
    		if( have_posts() ) {
    			...
    			while ( have_posts() ) {
					...
					if ( comments_open() ) {
					$items .= '<a href="'.$post_permalink.'#comment-area"><i class="fas fa-comments"></i><span>'. $post_comments .'</span></a> ';
					}
					if (function_exists( 'lip_love_it_link' )) {
					$items .= lip_love_it_link(get_the_ID(), '<i class="fas fa-heart"></i>', '<i class="fas fa-heart"></i>', false);
					}
					$items .= '</div>';				
					$items .= '</div>';
					$items .= '</li>';
...
SPBMap::map( 'recent_posts', array(
    "name"		=> __("Recent Posts", 'neighborhood'),
    "base"		=> "recent_posts",
